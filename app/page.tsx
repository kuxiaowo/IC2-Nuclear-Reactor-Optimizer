...
export default function Home() {
  ...
  const [fuelUsage, setFuelUsage] = useState<"exact"|"maximum">("exact");
  ...
  const [computeBackend, setComputeBackend] = useState<"scalar"|"numba_cpu"|"cuda">("numba_cpu");
  const [resultLimit, setResultLimit] = useState(10);
  const [search, setSearch] = useState({seconds:30,generations:100,population:80,workers:Math.max(1,(navigator.hardwareConcurrency||3)-2),seed:221,maxTicks:40000,unresolvedMaxTicks:140000});
  ...
  const runOptimization=async()=>{setError("");try{const payload={columns,fuel:{mode:fuelMode,usage:fuelUsage,...fuelLimits},component_limits:limits,marks,solver,compute_backend:computeBackend,result_limit:resultLimit,max_reactor_ticks:search.maxTicks,unresolved_max_reactor_ticks:search.unresolvedMaxTicks,cpu_workers:search.workers,...(solver==="heuristic"?{time_budget_seconds:search.seconds,generations:search.generations,population:search.population,seed:search.seed}:{})};if(solver==="exhaustive"){const estimateResponse=await fetch(`${API}/optimizations/estimate`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(payload)});if(!estimateResponse.ok)throw new Error((await estimateResponse.json()).detail||"无法计算枚举方案数");const estimate=String((await estimateResponse.json()).estimate);if(!window.confirm(`枚举方案数：${fmtIntegerString(estimate)}\n\n是否开始？`))return;setEnumerationEstimate(estimate)}else setEnumerationEstimate("");setJob(null);eventSource.current?.close();const res=await fetch(`${API}/optimizations`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(payload)});if(!res.ok)throw new Error((await res.json()).detail||"无法启动优化");const data=await res.json();followJob(data.id)}catch(e:any){setError(e.message)}};
  const pauseOptimization=async()=>{if(!job)return;setError("");const res=await fetch(`${API}/optimizations/${job.id}/pause`,{method:"POST"});if(!res.ok){setError((await res.json()).detail||"无法暂停任务");return}setJob(await res.json())};
  ...
  return <main onMouseUp={()=>setPainting(false)} onMouseLeave={()=>setPainting(false)}>
    ...
    {tab==="optimize"&&<div className="optimizer-layout"><section className="panel optimizer-form"><div className="panel-title">搜索约束</div><div className="form-grid"><label>反应堆仓<select value={columns-3} onChange={e=>resize(Number(e.target.value)+3)}>{[0,1,2,3,4,5,6].map(n=><option key={n} value={n}>{n}（{6*(n+3)} 格）</option>)}</select></label><label>燃料计数方式<select value={fuelMode} onChange={e=>setFuelMode(e.target.value as any)}><option value="separate">单/双/四联分别计数</option><option value="total_rods">按实际棒数合计</option></select></label><label>燃料数量规则<select value={fuelUsage} onChange={e=>setFuelUsage(e.target.value as any)}><option value="exact">恰好使用（推荐）</option><option value="maximum">最多使用</option></select></label>{fuelMode==="separate"?<><label>单联{fuelUsage==="exact"?"数量":"上限"}<input type="number" value={fuelLimits.single} onChange={e=>setFuelLimits(v=>({...v,single:Number(e.target.value)}))}/></label><label>双联{fuelUsage==="exact"?"数量":"上限"}<input type="number" value={fuelLimits.dual} onChange={e=>setFuelLimits(v=>({...v,dual:Number(e.target.value)}))}/></label><label>四联{fuelUsage==="exact"?"数量":"上限"}<input type="number" value={fuelLimits.quad} onChange={e=>setFuelLimits(v=>({...v,quad:Number(e.target.value)}))}/></label></>:<label>实际棒数{fuelUsage==="exact"?"":"上限"}<input type="number" value={fuelLimits.total_rods} onChange={e=>setFuelLimits(v=>({...v,total_rods:Number(e.target.value)}))}/></label>}</div>
        ...
        <div className="optimizer-options"><div><h3>目标 Mark（分别计算）</h3><div className="mark-pills">{["I","II","III","IV","V"].map(m=><button className={marks.includes(m)?"on":""} onClick={()=>toggleMark(m)} key={m}>Mark {m}</button>)}</div></div><div><h3>求解模式</h3><select value={solver} onChange={e=>setSolver(e.target.value as any)}><option value="heuristic">限时启发式最优</option><option value="exhaustive">穷举并证明全局最优</option></select></div><div><h3>计算后端</h3><select value={computeBackend} onChange={e=>setComputeBackend(e.target.value as any)}><option value="cuda">CUDA 大批量（小批自动 CPU）</option><option value="numba_cpu">Numba CPU 批量并行</option><option value="scalar">Python 标量参考</option></select></div><div><h3>输出结果数</h3><input type="number" min="1" max="100" value={resultLimit} onChange={e=>setResultLimit(Number(e.target.value))}/></div></div>
        {solver==="heuristic"&&<div className="form-grid search-tuning"><label>时间预算/秒<input type="number" min="1" value={search.seconds} onChange={e=>setSearch(v=>({...v,seconds:Number(e.target.value)}))}/></label><label>最大代数<input type="number" min="1" value={search.generations} onChange={e=>setSearch(v=>({...v,generations:Number(e.target.value)}))}/></label><label>种群规模<input type="number" min="10" value={search.population} onChange={e=>setSearch(v=>({...v,population:Number(e.target.value)}))}/></label><label>CPU 线程/进程<input type="number" min="1" max="64" value={search.workers} onChange={e=>setSearch(v=>({...v,workers:Number(e.target.value)}))}/></label><label>基础模拟周期<input type="number" min="2000" max="1000000" value={search.maxTicks} onChange={e=>setSearch(v=>({...v,maxTicks:Number(e.target.value)}))}/></label><label>竞争未决续算上限<input type="number" min={search.maxTicks} max="5000000" value={search.unresolvedMaxTicks} onChange={e=>setSearch(v=>({...v,unresolvedMaxTicks:Number(e.target.value)}))}/></label><label>随机种子<input type="number" value={search.seed} onChange={e=>setSearch(v=>({...v,seed:Number(e.target.value)}))}/></label></div>}
        {solver==="exhaustive"&&<div className="form-grid search-tuning"><label>CPU 线程/进程<input type="number" min="1" max="64" value={search.workers} onChange={e=>setSearch(v=>({...v,workers:Number(e.target.value)}))}/></label><label>基础模拟周期<input type="number" min="2000" max="1000000" value={search.maxTicks} onChange={e=>setSearch(v=>({...v,maxTicks:Number(e.target.value)}))}/></label><label>竞争未决续算上限<input type="number" min={search.maxTicks} max="5000000" value={search.unresolvedMaxTicks} onChange={e=>setSearch(v=>({...v,unresolvedMaxTicks:Number(e.target.value)}))}/></label></div>}
        <button className="primary large" disabled={!marks.length||["queued","running","paused"].includes(job?.status)} onClick={runOptimization}>{solver==="exhaustive"?"计算方案数并开始穷举":"开始计算最优布局"}</button>
      </section><section className="panel optimization-results"><div className="panel-title"><span>搜索结果</span><div>{job?.status==="running"&&<button onClick={pauseOptimization}>暂停并保存</button>}{job?.status==="paused"&&<button onClick={resumeOptimization}>继续计算</button>}{["running","paused"].includes(job?.status)&&<button onClick={()=>fetch(`${API}/optimizations/${job.id}/cancel`,{method:"POST"})}>取消</button>}{job&&solver==="heuristic"&&["completed","cancelled"].includes(job.status)&&<button onClick={resumeOptimization}>继续改进</button>}</div></div>{job?<><div className="progress-card"><div><strong>{job.message}</strong><span>{solver==="exhaustive"&&enumerationEstimate?`已检查 ${fmt(job.checked||0)} / ${fmtIntegerString(enumerationEstimate)} 个有标签方案`:`已评估 ${fmt(job.evaluated)} 个布局`} · {fmt(job.elapsed_seconds,1)} 秒</span></div><b>{Math.round(job.progress*100)}%</b><div className="progress"><span style={{width:`${job.progress*100}%`}}/></div>{job.status==="paused"&&<em>任务已暂停；当前进度已写入本地检查点，继续计算需要保持本次服务进程存活</em>}{job.proven_global&&<em>全部有标签布局均已模拟或由上界证明，结果为全局最优</em>}{!job.proven_global&&job.proven_within_horizon&&<em>有限模拟边界内的排列空间已闭合；仍有 {fmt(job.unresolved||0)} 个布局尚未获得无限期 Mark 结论</em>}</div>{marks.map(mark=><div className="leaderboard" key={mark}><h2>Mark {mark} <span>{job.leaderboards?.[mark]?.length||0} 个候选</span></h2>{job.leaderboards?.[mark]?.map((candidate:Candidate,i:number)=><button key={i} onClick={()=>loadCandidate(candidate,mark,i)}><b>#{i+1}</b><span><strong>{fmt(candidate.average_eu_per_tick,2)} EU/t</strong><small>{candidate.mark} · 安全 {fmt(candidate.safe_game_ticks)} tick · {candidate.component_count} 个组件</small></span><em>布局与曲线 →</em></button>)}{!job.leaderboards?.[mark]?.length&&<p>尚未找到精确匹配该 Mark 的可行布局。</p>}</div>)}</>:<div className="empty-results"><span>⚛</span><h2>等待搜索</h2><p>优化器会分别检查镜像方向，并为每个 Mark 的每组镜像布局保留最佳方向。排行榜只按平均 EU/t 排序。</p></div>}</section></div>}

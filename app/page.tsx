...
export default function Home() {
  ...
  const [search, setSearch] = useState({seconds:30,generations:100,population:80,workers:Math.max(1,(navigator.hardwareConcurrency||2)-1),resultLimit:1,seed:221,accelerator:"auto" as "auto"|"cpu"|"cuda"|"cuda_full",gpuBatchMultiplier:32,gpuExhaustiveBatchSize:8192,gpuTicksPerLaunch:256});
  ...
  useEffect(()=>{const max=Math.max(0,ROWS*columns-1);setLimits(current=>Object.fromEntries(Object.entries(current).map(([id,value])=>[id,Math.min(value,max)])))},[columns]);
  ...
  return <main onMouseUp={()=>setPainting(false)} onMouseLeave={()=>setPainting(false)}>
    ...
    {tab==="optimize"&&<div className="optimizer-layout"><section className="panel optimizer-form"><div className="panel-title">搜索约束</div><div className="form-grid"><label>反应堆仓<select value={columns-3} onChange={e=>resize(Number(e.target.value)+3)}>{[0,1,2,3,4,5,6].map(n=><option key={n} value={n}>{n}（{6*(n+3)} 格）</option>)}</select></label><label>燃料限制方式<select value={fuelMode} onChange={e=>setFuelMode(e.target.value as any)}><option value="separate">单/双/四联分别限制</option><option value="total_rods">实际棒数总上限</option></select></label>{fuelMode==="separate"?<><label>单联上限<input type="number" value={fuelLimits.single} onChange={e=>setFuelLimits(v=>({...v,single:Number(e.target.value)}))}/></label><label>双联上限<input type="number" value={fuelLimits.dual} onChange={e=>setFuelLimits(v=>({...v,dual:Number(e.target.value)}))}/></label><label>四联上限<input type="number" value={fuelLimits.quad} onChange={e=>setFuelLimits(v=>({...v,quad:Number(e.target.value)}))}/></label></>:<label>实际棒数上限<input type="number" value={fuelLimits.total_rods} onChange={e=>setFuelLimits(v=>({...v,total_rods:Number(e.target.value)}))}/></label>}</div>
        <h3>可用组件与库存上限</h3><div className="limit-grid">{components.filter(c=>!["empty","fuel"].includes(c.kind)).map(c=><label key={c.id} className={c.id in limits?"enabled":""}><input type="checkbox" checked={c.id in limits} onChange={()=>toggleLimit(c.id)}/><span className={`mini-icon kind-${c.kind}`}><ComponentIcon spec={c} small/></span><span>{c.name}</span><input type="number" min="0" max={Math.max(0,slots.length-1)} disabled={!(c.id in limits)} value={limits[c.id]??0} onChange={e=>setLimits(v=>({...v,[c.id]:Math.min(Math.max(0,Number(e.target.value)),Math.max(0,slots.length-1))}))}/></label>)}</div>
        <div className="optimizer-options"><div><h3>目标 Mark（分别计算）</h3><div className="mark-pills">{["I","II","III","IV","V"].map(m=><button className={marks.includes(m)?"on":""} onClick={()=>toggleMark(m)} key={m}>Mark {m}</button>)}</div></div><div><h3>求解模式</h3><select value={solver} onChange={e=>{const next=e.target.value as "heuristic"|"exhaustive";setSolver(next);if(next==="heuristic"&&search.accelerator==="cuda_full")setSearch(v=>({...v,accelerator:"cuda"}))}}><option value="heuristic">限时启发式最优</option><option value="exhaustive">穷举并证明全局最优</option></select></div></div>
        {solver==="heuristic"&&<div className="form-grid search-tuning"><label>时间预算/秒<input type="number" min="1" value={search.seconds} onChange={e=>setSearch(v=>({...v,seconds:Number(e.target.value)}))}/></label><label>最大代数<input type="number" min="1" value={search.generations} onChange={e=>setSearch(v=>({...v,generations:Number(e.target.value)}))}/></label><label>种群规模（CPU 精验）<input type="number" min="10" value={search.population} onChange={e=>setSearch(v=>({...v,population:Number(e.target.value)}))}/></label><label>结果数量<input type="number" min="1" max="10" value={search.resultLimit} onChange={e=>setSearch(v=>({...v,resultLimit:Number(e.target.value)}))}/></label><label>计算后端<select value={search.accelerator} onChange={e=>setSearch(v=>({...v,accelerator:e.target.value as "auto"|"cpu"|"cuda"|"cuda_full"}))}><option value="auto">自动（优先 CUDA）</option><option value="cuda">CUDA GPU</option><option value="cpu">仅 CPU</option></select></label><label>GPU 候选池倍数<input type="number" min="1" max="128" value={search.gpuBatchMultiplier} onChange={e=>setSearch(v=>({...v,gpuBatchMultiplier:Number(e.target.value)}))}/></label><label>CPU 精验进程<input type="number" min="1" max="64" value={search.workers} onChange={e=>setSearch(v=>({...v,workers:Number(e.target.value)}))}/></label><label>随机种子<input type="number" value={search.seed} onChange={e=>setSearch(v=>({...v,seed:Number(e.target.value)}))}/></label></div>}
        {solver==="exhaustive"&&<div className="form-grid search-tuning"><label>结果数量<input type="number" min="1" max="10" value={search.resultLimit} onChange={e=>setSearch(v=>({...v,resultLimit:Number(e.target.value)}))}/></label><label>穷举计算后端<select value={search.accelerator} onChange={e=>setSearch(v=>({...v,accelerator:e.target.value as "auto"|"cpu"|"cuda"|"cuda_full"}))}><option value="auto">自动（优先 GPU 证书）</option><option value="cuda">CUDA 固定点证书 + CPU 回退</option><option value="cuda_full">CUDA 完整模拟（实验）</option><option value="cpu">仅 CPU 多进程</option></select></label><label>GPU 穷举批大小<input type="number" min="256" max="131072" step="256" value={search.gpuExhaustiveBatchSize} onChange={e=>setSearch(v=>({...v,gpuExhaustiveBatchSize:Number(e.target.value)}))}/></label>{search.accelerator==="cuda_full"&&<label>每次 kernel 模拟周期<input type="number" min="16" max="2048" step="16" value={search.gpuTicksPerLaunch} onChange={e=>setSearch(v=>({...v,gpuTicksPerLaunch:Number(e.target.value)}))}/></label>}<label>CPU 模式工作进程<input type="number" min="1" max="64" value={search.workers} onChange={e=>setSearch(v=>({...v,workers:Number(e.target.value)}))}/></label></div>}

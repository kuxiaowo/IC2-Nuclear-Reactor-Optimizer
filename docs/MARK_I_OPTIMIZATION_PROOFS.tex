...
完整前沿 DP 尚未落地。当前实现使用更便宜的部分骨架乐观上界：已确定顶点与闭合边取精确值；每个未来顶点取当前可能类型的最大顶点值；每条未闭合边按已知端点取最大兼容值，两个端点未知时取全部类型对最大值；同一个剩余组件允许被乐观重复使用。任意真实后代的逐项贡献均不超过这些最大值，忽略库存竞争只会抬高结果，所以
\[

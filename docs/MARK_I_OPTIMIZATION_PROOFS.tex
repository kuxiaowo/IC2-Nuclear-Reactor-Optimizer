...
燃料组件数量或燃料棒总数的等式/上界约束以及反射板约束只作用于 $P$，冷却组件上限只作用于 $C$。由于两类组件不相交，完整布局满足库存约束当且仅当两层分别满足对应约束。
\end{corollary}
...
\begin{theorem}[排行榜下界剪枝]
若当前存在 10 个不同规范布局组，其功率均不小于 $\tau$，则任何骨架功率 $p<\tau$ 的冷却子树都不可能包含前十候选。
\end{theorem}

\begin{proof}
由骨架功率精确性，该子树中任一 Mark I-I 布局的功率均为 $p$。因为已有 10 个不同规范组的功率至少为 $\tau>p$，该布局不可能进入前十。算法不剪枝 $p=\tau$，故相同功率的稳定排序不受影响。
\end{proof}

\begin{theorem}[不会遗漏优解]
二层生成器不会遗漏任何可能进入 Mark I 前十的库存有效布局。
\end{theorem}

\begin{proof}
由双射定理，每个完整布局属于唯一骨架和冷却赋值。活动有限反射板骨架由引理证明不可行；低于门槛的骨架由排行榜下界定理证明不可能进入前十；其余骨架的冷却赋值全部生成并执行官方热学验证。因此所有可能进入前十的布局均被验证。
\end{proof}
...
令 $N$ 为完整布局数，$K$ 为实际构造的完整布局数，$M$ 为热学模拟数，$B$ 为批量跳过数，则
\[
...
设骨架成本、完整布局构造成本、热学模拟成本和额外调度成本分别为 $C_s,C_c,C_t,C_o$。近似有
\[
...
\]
所以可以无条件证明模拟数与完整布局构造数不增加，但不能无条件证明所有输入的墙钟时间严格缩短。
...
本节以后描述下一阶段拟实现的构造式求解器，而不是声称当前 \texttt{optimizer.py} 已包含前沿动态规划或约束合成器。当前二层生成器仍是完整性基线。新实现仅在通过本文交叉验证且所有未决分支闭合后，才可报告构造式全局最优。
...
\begin{theorem}[部分骨架热下界排除]
设部分骨架当前精确产热为 $Q_p$。对每个满足剩余精确燃料约束的封装选择 $z$，令 $B(z)$ 为新增燃料在零发电邻接下的固有产热，$F(z)$ 为其占用格数，$U(z)$ 为其余格在库存允许下的拓扑无关持续散热上界。若
\[
\forall z,\qquad Q_p+B(z)>U(z),
\]
则该部分骨架不存在 Mark I 完成。
\end{theorem}

\begin{proof}
新增燃料或反射板只会增加既有燃料的有效脉冲，因此最终产热不小于 $Q_p+B(z)$。$U(z)$ 把每个剩余格独立赋予最佳允许散热组件，并允许元件散热片拥有四个有效邻居，故不小于任何真实完成的持续外排。于是每个合法 $z$ 都满足最终产热严格大于持续外排上界，由持续热守恒定理均不可能为 Mark I。
\end{proof}

总棒数模式必须以有界背包联合保存 $F(z)$ 与 $B(z)$；分别独立取最低产热和最少占格会组合出未必存在的封装方案，只能形成更弱上界，不能用于上述实现中的精确权衡。
...
当前程序已实现第一组必要条件证书：\texttt{skeleton\_heat\_per\_tick} 精确计算 $Q(s)$；\texttt{\_partial\_mark\_i\_heat\_infeasible} 在骨架完成前联合剩余燃料固有产热与拓扑无关散热上界；\texttt{sustainable\_vent\_upper\_bound} 计算骨架层乐观持续外排；\texttt{sustainable\_heat\_flow\_upper\_bound} 在完整冷却赋值上建立乐观最大流。该网络保留燃料热源、散热上限、元件散热邻接、抽堆热和换热硬上限，同时放宽行优先顺序、热比例和容量。只有最大流仍小于产热时才排除。
...
完整前沿 DP 尚未落地。当前实现使用更便宜的库存感知定向脉冲上界：已确定顶点与闭合边取精确值；一条边拆成两个燃料端点各自向发电邻居贡献的定向脉冲；已处理燃料对跨越前沿的边乐观假设未来端能够响应；未来固定燃料按实际度数计算，未来非固定燃料一律赋予四个发电邻居；剩余燃料选择服从组件包上限，总棒数模式再以有界背包服从剩余棒数。任意格点实际度数不超过四，每条真实边的两个定向贡献均被覆盖，且背包在全部合法剩余燃料选择上取最大值，所以
\[
P(L)\le U_{\mathrm{partial}}.
\]
程序只在 $U_{\mathrm{partial}}$ 严格低于排行榜门槛时剪枝。

设被剪枝前缀还有 $m$ 个非固定骨架位置，第 $t$ 种组件再使用 $n_t$ 个，则其有标签位置数为
\[
\frac{m!}{(m-\sum_t n_t)!\prod_t n_t!}.
\]
\texttt{count\_remaining\_layouts} 以等价的逐类型组合递推跟踪总棒数和燃料存在性，并对最终空位数乘以 \texttt{count\_cooling\_completions}，故它精确计数整个被剪枝完整布局子树。
...
算法维护具有精确 DP 功率上界 $U$ 的未决子问题优先队列，以及已经精确验证的可行下界 $P^*$。骨架子问题只按固定槽位或库存限制等可由 DP 状态表达的条件分裂；冷却 no-good 只排除当前固定骨架下已经精确验证失败的冷却赋值，不改变骨架上界。每次处理最大 $U$ 的子问题，依次应用有限反射板、热守恒、槽位下界和必要条件松弛；候选由精确模拟复核。无法证明的分支最终回退到完整冷却生成。当最大未决上界满足 $U_{\max}\le P^*$ 时停止。
...
部分骨架功率上界 & \texttt{optimistic\_power\_bound}（二层生成器内部） \\
部分骨架完整计数 & \texttt{count\_remaining\_layouts}（二层生成器内部） \\
拟实现：骨架功率 DP & 前沿 DP 的顶点项、边项与库存状态 \\
拟实现：冷却约束合成 & 必要条件松弛；\texttt{SAT} 后调用精确模拟 \\
拟实现：最优停止证书 & $U_{\max}\le P^*$ 且未决分支为空 \\
\bottomrule
\end{longtable}

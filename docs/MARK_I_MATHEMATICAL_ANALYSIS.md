...
每个状态最多尝试 $k$ 个标签。该界仍可能很大，因此当前实现不再使用固定列数阈值，而在以下条件同时满足时启用：

1. 实际前沿标签状态上界 $k^w\le 4096$；
2. 剩余可变槽位不超过 18；
3. 功率组件类型不超过 6。

该判据保留原来最坏情况下的 4 列范围，因为 $7^4=2401$，同时允许小标签库存扩展到 6/9 列。例如只有空槽和一种燃料时，9 列前沿仅有 $2^9=512$ 种标签组合。启用条件只决定是否计算第二级上界，不参与布局合法性判断；超预算时回退仍然完备。

缓存最多保留 65,536 个状态。超出范围或尚未形成 Top-K 下界时不运行 DP，继续使用原有定向脉冲上界和完整 DFS。跳过一个上界检查只会减少剪枝，不会改变搜索空间或最优性证明。

### 18.6 可观测统计

每个 Mark I shard 返回：DP 剪枝调用数、DP 排序调用数、缓存命中/未命中数、DP 剪枝节点及其折算布局数，以及第一级松弛上界对应的剪枝数量。主任务汇总后通过 `power_frontier_stats` 暴露。统计不参与任何剪枝判断。

## 19. 批量编译热流必要条件

完整冷却布局原先逐个在 Python 中构造第 9 节的松弛热流网络并运行 Dinic。当前实现先把至多一个搜索批次的布局编码为固定宽度 `uint8` 矩阵，再由 Numba 内核逐行构造同一个网络并运行整数 Edmonds–Karp。枚举工作进程本身已经进程并行，因此批次内部保持单线程，避免嵌套线程池争用。

### 19.1 网络等价性

对一个布局 $L$，记 Python 实现构造的容量多重图为 $G_L=(V,E,c)$。Numba 实现使用稠密剩余矩阵，并令：

$$
C(u,v)=\sum_{e\in E:u\to v}c(e)
$$

即把同方向平行边容量相加。任意多重图流 $f$ 可按各平行边容量拆分为矩阵图上的流，反之矩阵边上的流也可依次分配回平行边，因此两者的可行流值集合相同，最大流相同。

两份实现逐项使用相同规则：

1. 燃料源容量为 $2r p(p+1)$；
2. 燃料到堆芯及可受热邻居的松弛容量为总产热 $H$；
3. 自散热、堆芯抽热和邻接散热分别使用 `self_vent`、`hull_draw`、`side_vent`；
4. 交换器使用相同的双向邻接容量和 $\max(exchange\_hull,\lfloor exchange\_side/2\rfloor,1)$ 堆芯容量；
5. 源点、汇点、槽位节点和邻接表一一对应。

所以 Numba 稠密图与 Python 多重图最大流严格相等。

### 19.2 增广算法正确性与提前结束

Edmonds–Karp 每次在正剩余容量图中用 BFS 找到一条增广路。若找不到增广路，由最大流最小割定理，当前流为最大流；若找到则按路径瓶颈容量增广并保持容量限制与流守恒。所有容量为非负整数，故算法有限步终止并返回整数最大流。

该网络从源点发出的容量和恰好等于骨架总产热 $H$，所以最大流不可能超过 $H$。内核达到 $H$ 时立即返回并不近似：此时已经构造出值为源割容量的可行流，故该流必为最大流。

于是对每个批次行都有：

$$
F_{Numba}(L)=F_{Python}(L)
$$

程序仍然只在 $F(L)<H(L)$ 时剪枝。批处理只改变执行位置和数据表示，不改变不等式。若 Numba 导入、编译或运行失败，程序永久降级为该进程内的逐布局 Python 网络；编译失败不会被解释成不可行证书。

### 19.3 差分测试与局部基准

固定随机种子的差分测试覆盖 3、6、9 列以及燃料、反射板、散热器、交换器和空槽的混合布局，逐值比较两个实现的最大流，而不只比较最终是否通过。

在本机已预热 JIT、单进程、4,096 个 3 列随机布局的局部基准中，包含布局编码的批处理耗时由 Python 逐布局的 0.0961 秒降为 0.0066 秒，约 14.65 倍。该数字只说明热流过滤器本身的测量结果，不承诺完整搜索等比例加速。

## 20. 上界引导的枚举顺序

### 20.1 骨架子分支

当第 18 节 DP 处于受控状态预算内时，程序对当前骨架节点的每个合法子选择 $t$ 计算同一个精确后代功率上界 $U_t$，再按 $U_t$ 降序递归。形成不可持续有限反射板边的子选择放在正常子选择之后。这里的 $U_t$ 只作为排序键；子树是否关闭仍然只由原有的 $U_t<\tau$ 严格不等式决定。

设原子选择序列为 $(t_1,\ldots,t_m)$，排序后为其置换 $(t_{\pi(1)},\ldots,t_{\pi(m)})$。各子树由当前位置标签区分，彼此不交，且并集不变：

$$
\bigcup_{j=1}^{m}S_{t_j}
=
\bigcup_{j=1}^{m}S_{t_{\pi(j)}}
$$

因此排序不能增加或删除布局。其作用只是更早取得高功率可行见证，提高后续安全 Top-K 下界。

### 20.2 冷却分支与 shard

冷却组件按乐观每 tick 排热能力和输运能力降序，空槽仍被保留；shard 按固定中心格燃料的当前功率及未固定邻居脉冲潜力降序。两者都只对有限列表做稳定置换，不把排序分数用于剪枝。由与上式相同的集合并集论证，完整性、最优性和布局计数均不变。

6/9 列回归测试使用两根精确单铀库存，验证自适应 DP 被启用、DP 排序确实发生、最优功率仍为 20 EU/t，且：

$$
\texttt{checked}=\texttt{evaluated}+\texttt{pruned}
=\texttt{estimate\_exhaustive\_space}
$$

## 21. 暂未接入的功率–产热 Pareto DP

功率是顶点和边的可加函数，前沿只需保存组件标签。产热则为：

$$
H_v=2r_vp_v(p_v+1)
$$

它关于最终邻接脉冲数 $p_v$ 为二次函数。一个尚在前沿中的燃料除了类型外，还必须保存已经确定的活跃邻居数；否则两个标签相同但已累积脉冲不同的前缀会被错误合并。若有 3 种燃料、3 种反射板，并为每种燃料保留 0 至 4 的脉冲计数，单个前沿位置已有至多：

$$
1+3+5\times3=19
$$

种局部状态。即使改用固定高度 6 的列扫描，标签前沿上界也达到 $19^6=47,045,881$；9 格宽行前沿则为 $19^9=322,687,697,779$。这还没有乘库存、棒数和每个状态中的非支配 $(P,H)$ 对数量，也没有表达冷却空槽拓扑。

因此当前版本没有把“最大功率对应的最小产热”当作整棵子树证书：最高功率点热不可行，并不能推出稍低但仍高于排行榜门槛的点也不可行。只有完整 Pareto 前沿并同时给出对冷却能力的可靠上界时才能安全剪枝。现阶段保留已有的独立功率上界、产热下界、部分冷却上界和完整布局最大流，避免用不充分状态误剪。

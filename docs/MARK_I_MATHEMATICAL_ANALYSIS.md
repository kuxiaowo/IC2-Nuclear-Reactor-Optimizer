...
| 前沿动态规划可直接构造最大功率骨架 | 成立且已实现 | 网格目标仅含顶点项和相邻边项 | `SkeletonPowerTable` 持久表与最大堆替代逐槽骨架枚举 |
...
### 5.6 可配置 Top-$K$ 排行榜功率下界

令用户请求的结果数量为 $K=\texttt{result\_limit}$，其中 $1\le K\le10$。当已经找到 $K$ 个不同规范布局组时，设当前第 $K$ 名功率为 $\tau_K$。对 $K>1$，代码只在骨架功率上界 $U$ 满足以下严格不等式时剪枝：

$$
U<\tau_K
$$

已经存在 $K$ 个不同规范布局组，其功率均不低于 $\tau_K$。同一完整骨架下任何 Mark I-I 冷却布局的功率都是 $p\le U$，因此当 $U<\tau_K$ 时，该子树不可能进入 Top-$K$。

代码在 $K>1$ 时不剪枝 $U=\tau_K$，所以相同功率下的规范布局稳定排序不会被遗漏。

当 $K=1$ 时，目标只是返回**任意一个**全局最高功率规范组。找到首个候选并令其功率为 $\tau_1$ 后，满足 $U\le\tau_1$ 的子树都不可能产生更高功率，因而等号也可安全剪枝。若产品语义要求在并列最优中再取规范键最小者，则必须恢复严格不等式；当前 Top-1 接口明确保证全局最优功率，不保证特定的并列代表。

这正是“先找第一个”的有效形式：不是完成一次 Top-1 后重新枚举 Top-2，而是在同一棵搜索树中尽早建立门槛。直接串行重跑 $K$ 次会重复访问公共前缀，通常更慢。

### 定理 B3：二层生成器不会漏掉所请求的 Top-$K$ 优解

对任意库存有效完整布局 $L$，由定理 B1，它会落入唯一的骨架和冷却赋值：

1. 若骨架含活动有限反射板，则 $L$ 不可能是 Mark I-I；
2. 若骨架功率上界低于已证明的第 $K$ 名门槛（Top-1 时不高于第一名门槛），则 $L$ 不可能改善请求结果；
3. 其余骨架的所有冷却赋值都会被第二层生成并按官方热学逻辑验证。

因此所有可能进入 Mark I Top-$K$ 的布局都被实际验证；Top-1 至少保留一个达到全局最大功率的见证。证毕。

### 5.7 多进程共享门槛的安全性

第 $K$ 名门槛只会保持或升高，不会降低。工作进程可能暂时看到旧的较低门槛，这只会多做热学模拟，不会错误剪枝。进程本地门槛来自该进程已经找到的 $K$ 个不同规范组；这些候选同样是全局可行候选，因此本地门槛也可作为安全下界。$K=1$ 时首个可行候选即可立即向全部进程发布门槛。
...
| 请求名次数 | `result_limit` | Top-1 更早启用门槛并允许等号剪枝 |
...
这是完整布局最终验证次数的上界。简单固定点证书失败时可能先增加至多 4 次短状态转移再回退长模拟，所以该不等式不等于墙钟成本必然下降。

### 6.3 基础生成器的完整布局构造次数不会增加
...
只要至少有一棵非空冷却子树被批量排除，就有 $K<N$。加入冷却见证构造器后，还会额外尝试至多常数个定向见证，因此“总构造操作次数不增加”不再是无条件结论；Top-1 见证成功时省去的大冷却子树通常远大于这些尝试，但失败输入可能产生小额额外开销。
...
- **可证明：** 完整布局最终验证不重复，正常完成仍覆盖全部计数；
- **不可无条件证明：** 所有输入的墙钟时间都严格减少。

最坏情况下没有任何骨架可剪枝且构造证书全部失败，新算法会增加骨架分析和 $WC_w$ 开销。最佳情况下，高功率可行骨架很早获得构造见证并建立门槛，大量同骨架冷却排列和低功率骨架子树都由组合数直接跳过。
...
`prove_simple_fixed_point` 只处理不含换热器、冷却单元和冷凝模块的简单完整布局。它从零热状态出发，最多执行 4 次与生产模拟器完全相同的 `ReactorSimulator.step`；若沿安全前缀到达状态 $x$，且下一次状态转移满足

$$
F_L(x)=x,
$$

则由确定性和归纳法，布局以后永久重复同一安全状态。证书同时检查临界、非燃料组件损坏和融毁，并保留完整有方向布局作为键，不能跨镜像复用。为了保持既有产品语义，只有验证边界至少覆盖两个完整燃料周期时才直接返回 Mark I-I 结果。

该函数返回 `None` 只表示“简单证书无法证明”，不表示布局不可行；统一入口 `_evaluate_layout_uncached` 随即回退到原完整模拟器。因此把证书接入启发式、普通穷举和 Top-$K$ 评估不会减少搜索覆盖。

### 7.2 已实现的冷却见证构造器

`construct_simple_cooling_candidates` 根据每个燃料位置的精确产热，优先把库存中的直接自散热组件放到燃料邻接热量较高的位置，并尝试相反行优先方向和堆体抽热方向。构造结果只有通过上述固定点证书后才成为候选。

构造器是见证查找器，不是不可行判定器：

1. 构造成功且证书通过：提前得到可行下界；
2. 构造失败或证书返回未知：继续原冷却完整生成；
3. Top-1 中，一个证书见证已经实现该完整骨架的精确功率，其余同骨架冷却完成只能并列，故可计数后整棵跳过；
4. Top-$K$（$K>1$）中，证书候选只用于提前填榜，原生成器继续处理同功率并列布局；已证布局到达叶节点时不重复验证。

因此构造器不能造成漏解。它只把“较早找到一个可行布局”加入现有完整搜索，所有未知情况仍由完整回退闭合。
...
| 简单可达固定点证书 | `prove_simple_fixed_point` | 最多 4 次精确状态转移证明永久稳定 |
| 冷却见证构造器 | `construct_simple_cooling_candidates` | 提前构造并证明直接散热布局 |
...
11. 简单证书结果与完整模拟器的 Mark、EU/t、安全时长和安全裕量一致；
12. 人工令构造器失败后，完整回退仍覆盖相同计数并得到相同全局最优值；
13. Top-1 构造见证关闭冷却子树后仍满足 `evaluated + pruned = checked`。
...
本节到第 15 节给出构造式求解器的程序契约、正确性证明和复杂度边界。第 7 节的简单直连冷却见证构造器和第 14 节的前沿动态规划、精确功率优先队列均已实现；通用外部约束合成器仍是拟实现设计。热学无法证明时仍由第 5 节的完整冷却生成器回退验证。
...
### 14.2.1 当前已实现的精确后缀功率表

`SkeletonPowerTable` 按列逐格扫描。反应堆高度固定为 6，因此前沿始终只有 6 个标签。对状态

$$
H(i,F,\mathbf r)
$$

记录从第 $i$ 格开始、前沿为 $F$、剩余库存为 $\mathbf r$ 时能够增加的**精确最大功率**。固定发电前缀只允许指定标签；功率层空位哨兵 `POWER_EMPTY` 强制该格在骨架中为空，但不占用完整布局槽位，第二层仍可在此放置冷却组件。总棒数约束由已用库存唯一推出，不需要额外近似。
...
终态只有在至少使用一个燃料组件时取 0，否则取负无穷。该表不再允许同一库存被重复乐观使用，所以它替代了旧的 `optimistic_power_bound`；骨架主路径也不再调用逐槽 DFS `generate_skeleton`。

在“总棒数”模式下，单联、双联和四联没有各自库存上限，三者共享一个“剩余棒数”资源；程序不会再把三种包装的累计使用量作为三个独立 DP 维度。反射板等具有独立上限的发电组件仍各占一个资源维度。

表以规则版本、表结构版本、Python marshal 版本、列数、发电组件顺序与上限、总棒数约束、固定骨架槽为键，压缩保存到 `.data/skeleton_power_tables.sqlite3`。再次遇到同一子问题时直接加载全部状态。冷却库存、热学结果和镜像可行性不写入该表，因为它们不是骨架功率状态的一部分。

### 定理 E1.1：最大堆按非增功率产生骨架
...
取出最大 $U$ 的节点；若它未完成，就用所有合法下一标签把该前缀的后代集合拆成互斥且完备的子集合，并给每个子节点计算精确界。若取出的是完整骨架，则 $U=P$。此时队列中任一未决骨架的功率不超过其所在节点的界，也不超过当前 $P$，所以输出序列严格按非增功率排列。第一项是最大功率骨架；继续弹出即可得到第二个布局。若要求第二个**不同功率值**，只需跳过与第一项同功率的完整骨架。

当 $K>1$ 时，只有节点精确界严格低于第 $K$ 名门槛才整棵排除；相等时保留同功率规范布局组。Top-1 已有见证时，$U\le\tau_1$ 的节点不能改善目标，可以连同其全部冷却完成一次计数并关闭。`count_power_subtree` 对每个互斥队列前缀递推计算完整有标签布局数，因此仍保持 `evaluated + pruned = checked`。

### 14.2.2 当前已实现的自适应功率前缀分片

旧分片把冷却组件也固定进完整布局前缀。这样虽然容易制造大量任务，却会让同一发电骨架的不同冷却完成落入不同分片，导致功率表和骨架优先队列重复构建。Top-1 的冷却见证本来可以一次关闭整个骨架子树，这种拆分反而破坏了局部性。

当前 `_exhaustive_shards` 改为：

1. 按距网格中心由近到远选择固定位置；
2. 分片前缀只允许发电组件和功率层空位 `POWER_EMPTY`；
3. 每增加一个固定位置就生成满足库存与总棒数约束的全部前缀；
4. 当前缀数量达到 `cpu_workers × 4` 或位置耗尽时停止展开；
5. 工作进程数仍取请求值与实际分片数的较小者。

### 定理 E1.2：自适应功率前缀分片互斥且完整

设功率投影 $\pi(L)$ 保留燃料与反射板，并把其他标签统一映射为 `POWER_EMPTY`。对选中的固定位置集合 $R$，任意完整有标签布局 $L$ 都有唯一功率投影 $\pi(L)|_R$；分片器枚举全部库存合法的功率投影，所以每个完整布局恰好属于一个功率前缀分片。

Mark I 第一层把固定发电组件写入骨架；`POWER_EMPTY` 只禁止发电组件，不从冷却空位数中扣除。第二层因此仍能在这些位置放置任意库存合法冷却组件。不同功率前缀互斥，每个前缀内部保留完整的“骨架、冷却完成”双射。

同一发电骨架及其全部冷却完成现在只属于一个分片，因此功率表不会因冷却前缀而复制。多个工作进程之间仍通过共享排行榜下界剪枝；可用并行度由互斥功率前缀数决定，而不再靠拆散冷却完成虚增任务数。

### 14.3 带上界的可实现骨架搜索

单独回溯一个 $P_{\max}$ 骨架并不完备，因为该骨架可能不可实现。当前每个分片维护按精确功率上界降序排列的未决前缀队列：

1. 用前沿动态规划计算每个子问题的精确骨架上界 $U$；
2. 取出 $U$ 最大的子问题并回溯一个最大骨架；
3. 应用有限反射板、热守恒、槽位和约束松弛证书；
4. 直接冷却构造失败时，回退到该骨架的完整冷却生成；
5. 每个完整冷却布局先过乐观热流证书，无法排除时运行精确模拟；
6. 找到 Mark I 后更新当前可行下界 $P^*$；
7. 当所有未决子问题的最大上界 $U_{\max}\le P^*$ 时停止。

骨架子问题通过给下一个尚未决定槽位选择一种合法标签进行分裂。子节点对应互斥前缀，且其并集恰好覆盖父节点。冷却失败不改变骨架 DP 上界，只关闭已验证的冷却赋值；同一骨架的其他冷却完成仍由回退生成器覆盖。
...
持久化不改变第一次建表的上述渐近复杂度；命中缓存时只需读取并解压实际到达的状态表。表项是“前沿状态的一个整数最优值”，不是完整骨架列表。库存维数很多或上限接近 54 时，$B$ 仍可能很大，所以不能把“可持久化”误解为能够预生成从 1 到 216 棒的全部位置布局。
...
简单冷却构造器另有一个 Top-1 单进程隔离基准：3 列、单铀 1 个、普通散热片上限 4 个，共 57,852 个完整布局。关闭构造器时每次约 18.1 ms；开启后约 0.78 ms，二者均检查 57,852 个布局、精确验证 1 个、数学跳过 57,851 个且最优值均为 5 EU/t。约 23 倍的差异主要来自构造见证后不再递归展开同骨架的冷却排列；这是进程内微基准，不代表含换热器或 Top-$K$ 请求的统一倍数。

功率层分片的结构回归测试中，3 列、单铀上限 4、请求 31 个进程时，在第 8 个中心优先位置得到 163 个互斥功率前缀，足以启用 31 个进程，且前缀中不含冷却标签。若只有一根单铀，则完整功率空间本来就只有 18 个含燃料骨架和一个无燃料骨架；此时最多生成 19 个分片，不再为凑满核心而复制冷却搜索。

### 15.6 持久功率表基准

9 列、最多 4 个单联铀燃料、Top-1、单进程的隔离测试中，旧逐槽骨架路径需要处理 342,540 个完整骨架，且在无候选下反复计算部分上界，约耗时 63.1 秒。当前实现有两类结果：

| 输入 | 首次建表 | 再次命中 | DP 状态 | 结果 |
|---|---:|---:|---:|---|
| 反应堆散热片上限 20，可构造最高功率见证 | 约 0.033 s | 约 0.008 s | 6,272 | 首个骨架 60 EU/t，精确验证 1 个后闭合 Top-1 |
| 无任何持续散热组件 | 不建功率表 | 约 0.001 s | 0 | 根级热守恒一次证明 342,540 个骨架均不可实现 |

第一行证明了持久表和精确功率顺序在该稀疏库存上的收益；第二行主要来自与新骨架入口结合的全局最低产热/最大持续散热比较，不应把全部倍数归因于 DP。若大量同功率高功率骨架都无法由证书关闭，最大堆仍可能逐个产生它们，墙钟收益会低于本表。
